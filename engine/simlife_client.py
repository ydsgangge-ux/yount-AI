                ...
                if _alog:
                    ...
                    for entry in recent:
                        ...
                        if et == "action":
                            ...
                            # 主语：AI角色和用户角色一起
                            _ai_name = char.get("name", "?") if char else "?"
                            _uc_name = user_char.get("name", "用户") if user_char else "用户"
                            subject = f"{_ai_name}和{_uc_name}"
                            if act:
                                act = f"{subject} {act}"
                            # 旧英文键 → 中文键
                            KEY_ZH = {
                            ...
                            if act:
                                lines.append(f"  [{t}] 🧰{skill}：{act}" + (f"（{'、'.join(detail_parts)}）" if detail_parts else ""))
        except Exception:
            pass
